...
import { Newspaper, MessagesSquare, Trash2, ExternalLink } from 'lucide-react'
import { setBlogPublished, deleteBlogPost, deleteCommunityPost, deleteCommunityComment } from '@/app/admin/actions'
    ...
    <div className="stack">
      ...
      <div style={{ display: 'inline-flex', gap: 4, background: 'rgba(28,43,58,.05)', padding: 4, borderRadius: 10, alignSelf: 'flex-start' }}>
        {([['blogs', `Blogs (${blogs.length})`, <Newspaper key="b" size={15} />], ['community', `Community (${community.length})`, <MessagesSquare key="c" size={15} />]] as const).map(([k, lbl, icon]) => (
          <button key={k} onClick={() => setTab(k as 'blogs' | 'community')} style={{

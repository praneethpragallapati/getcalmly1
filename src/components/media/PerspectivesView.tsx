 ...
 */
export function PerspectivesHero({ comingSoon = true }: { comingSoon?: boolean }) {
  return (
...
export function PerspectivesView({ sections, onDark = true, showHero = true }: { sections: PerspectiveSectionView[]; onDark?: boolean; showHero?: boolean }) {
  const anyComingSoon = sections.some((s) => s.comingSoon)
  return (
    <div>
      {showHero && <PerspectivesHero comingSoon={anyComingSoon} />}

...
type Sort = 'name' | 'sessions' | 'attention'
...
export function CaseloadTable({ patients }: { patients: CaseloadPatient[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return list
  }, [patients, q, pkg, lang, minSessions, sort])

  const activeFilters = [pkg, lang, minSessions].filter(Boolean).length + (q.trim() ? 1 : 0)
      ...
      <div className="card" style={{ display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'flex-end' }}>
        ...
        {activeFilters > 0 && (
          <button className="btn" style={{ border: '1.5px solid #E2E8F0', fontSize: 12.5, padding: '9px 12px' }}
            onClick={() => { setQ(''); setPkg(''); setLang(''); setMinSessions(''); setSort('name') }}>
            Clear filters
          </button>
        )}
      </div>
            ...
            <Link key={p.patientId} href={`/expert/patients/${p.patientId}`} className="pattern" style={{ textDecoration: 'none' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div className="pattern-sub">
                  {p.trackLabel} · Mood {p.moodTrend} · {p.sessionsCompleted} completed · Wallet {p.sessionsDone}/{p.sessionsTotal}
                </div>
              </div>
              {p.openCrisisCount > 0 && (

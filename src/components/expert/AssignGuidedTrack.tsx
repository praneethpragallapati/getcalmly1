...
export function AssignGuidedTrack({
  patientId, tracks, assignments, readOnly = false,
}: {
  patientId: string
  tracks: { id: string; title: string }[]
  assignments: Assignment[]
  /** Supervisor view: show what's assigned, but don't offer to change it. */
  readOnly?: boolean
}) {
  ...
  return (
    <div className="card">
      ...
      <p className="muted" style={{ fontSize: 12.5, marginBottom: 12 }}>
        {readOnly
          ? 'Guided tracks the treating clinician has assigned to this patient.'
          : 'Assign a guided track to this patient. Leave the date empty for no expiry.'}
      </p>

      {readOnly && assignments.length === 0 && (
        <p className="muted" style={{ fontSize: 13 }}>No guided tracks assigned.</p>
      )}

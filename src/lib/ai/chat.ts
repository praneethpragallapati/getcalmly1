/**
 * getCalmly AI chat pipeline (#11). Ported from the v6 classified-routing notebook:
...
export async function runChat(userId: string, question: string): Promise<ChatResult | null> {
  ...
  if (!cfg.features[userType].chatbot) {
    const msg = 'getCalmly AI chat is not available on your plan right now.'
  ...
  if (cap > 0 && (await monthlyTokensFor(userId)) >= cap) {
    const msg = "You've reached this month's usage limit for getCalmly AI. It resets at the start of next month."
